...
if [[ $zk_list_size -eq 2 ]] ; then
  (grep -q '^default.replication.factor' ${CONFIG} && sed -i 's|^default.replication.factor=.*$|default.replication.factor=2|' ${CONFIG}) || echo -e "\ndefault.replication.factor=2" >> ${CONFIG}
fi
