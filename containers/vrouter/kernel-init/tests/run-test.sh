...
/opt/contrail/vrouter-kernel-modules/4.18.0-305.12.1.el8_4.x86_64/vrouter.ko"
...
/lib/modules/4.18.0-305.3.1.el8_4.x86_64
/lib/modules/4.18.0-305.12.1.el8_4.x86_64"
...
[[ "${RESULTS["3.10.0-1062.el7.x86_64"]}" == '3.10.0-1062.1.1.el7.x86_64' ]] || FAILED_KERNELS+=('3.10.0-1062.el7.x86_64')
...
[[ "${RESULTS["4.18.0-305.3.1.el8_4.x86_64"]}" == '4.18.0-305.12.1.el8_4.x86_64' ]] || FAILED_KERNELS+=('4.18.0-305.3.1.el8_4.x86_64')
[[ "${RESULTS["4.18.0-305.12.1.el8_4.x86_64"]}" == '4.18.0-305.12.1.el8_4.x86_64' ]] || FAILED_KERNELS+=('4.18.0-305.12.1.el8_4.x86_64')

# Report
set +x
if [ "${#FAILED_KERNELS[@]}" -ne '0' ] ; then
  echo "ERROR: An inappropriate module is proposed for kernels:"
  for i in ${FAILED_KERNELS[*]} ; do
    echo "$i" - "${RESULTS[$i]}"
  done
  exit 1
else
  echo "INFO: Test for containers/vrouter/kernel-init is success"
fi
