...
for phys_int in $phys_int_list; do
    ...
    if [[ -n "$vlan_data" ]] ; then
        ...
        cmd+=" --vlan_tci ${vlan_id} "
        if is_enabled "$DPDK_ENABLE_VLAN_FWRD" ; then
...
if [[ -n "$L3MH_CIDR" ]]; then
    vhost0_init_function='init_vhost0_l3mh'
else
    vhost0_init_function='init_vhost0'
fi

export CONTRAIL_DPDK_CONTAINER_CONTEXT='true'
for i in {1..3} ; do
    echo "INFO: $vhost0_init_function ... $i"
    $vhost0_init_function && break
    if (( i == 3 )) ; then
