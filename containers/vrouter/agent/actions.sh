...
function start_agent() {
    ...
    # vrouter agent config available later sometimes, e.g. when k8s operator prepares it.
    # That is why we have to wait for the config file available.
    # We can't get further because we can't run the agent service without the config.
